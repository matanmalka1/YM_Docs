...
backend/app/
  ...
  audit/
  users/
  permissions/              # target — may stay inside users/core if thin
  imports/                  # target — client import only
```

המבנה הפנימי של כל תיקייה (services/, repositories/, routers/, schemas/) הוא convention נפרד ולא חלק ממסמך זה.
