...
8. **Reference counts corrected + scoped** (Rounds 2–3): legacy-model **class**-reference files = **20** (word-boundary grep on the 5 model names); legacy repo/schema/route/consumer files = **13**; audit test files = **38** (the list had 38 paths, mislabelled 29). The earlier single "29" conflated two different sets — now each set has its own exact count + list (§17).
...
| Legacy-model **class**-reference files (non-test, word-boundary grep on the 5 model names) | 20 | §17 |
| Legacy repo/schema/route/consumer files (non-test) | 13 | §17 |
...
**Scope note (counting):** two distinct sets, each labelled with its own exact count.

**(a) Legacy-model CLASS-reference files (non-test, 20)** — files that name one of the 5 legacy model classes (`\bVatAuditLog\b|\bAnnualReportStatusHistory\b|\bBinderLifecycleLog\b|\bBinderIntakeEditLog\b|\bSignatureAuditEvent\b`): annual_reports/models/__init__.py, annual_reports/models/annual_report_status_history.py, annual_reports/repositories/annual_report_status_audit_repository.py, binders/models/binder_intake_edit_log.py, binders/models/binder_lifecycle_log.py, binders/repositories/binder_intake_edit_log_repository.py, binders/repositories/binder_lifecycle_log_repository.py, binders/services/binder_audit_service.py, dashboard/services/dashboard_recent_activity_service.py, seed/builders/demo/binders.py, seed/builders/demo/reports.py, seed/builders/demo/signature_requests.py, seed/builders/demo/vat.py, signature_requests/models/signature_request.py, signature_requests/repositories/signature_request_audit.py, signature_requests/repositories/signature_request_repository.py, timeline/repositories/timeline_repository.py, vat/models/vat_audit_log.py, vat/repositories/vat_audit_log_repository.py, vat/repositories/vat_work_item_write_repository.py.

**(b) Legacy repo/schema/route/consumer files (non-test, 13)** — files that consume the legacy repos/schemas/routes without naming the model class directly: annual_reports/repositories/annual_report_repository.py, annual_reports/repositories/annual_report_status_audit_repository.py, binders/repositories/binder_intake_edit_log_repository.py, binders/repositories/binder_lifecycle_log_repository.py, binders/services/binder_audit_service.py, binders/services/binder_intake_edit_service.py, binders/services/binder_lifecycle_service.py, dashboard/services/dashboard_recent_activity_service.py, signature_requests/repositories/signature_request_audit.py, signature_requests/repositories/signature_request_repository.py, timeline/services/timeline_service.py, vat/repositories/vat_audit_log_repository.py, vat/repositories/vat_work_item_write_repository.py. Additional consumer surfaces touched during the refactor (schemas/routes/builders, overlapping with (a)/(b)): vat/api/vat_routes_queries.py, vat/schemas/vat_audit.py, annual_reports/schemas/annual_report_responses.py, signature_requests/schemas/signature_request.py, signature_requests/signature_request_response_builder.py, model_registry.py, seed/orchestrator.py, timeline/timeline_binder_event_builders.py.
