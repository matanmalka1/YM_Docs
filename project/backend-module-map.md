...
| `app/core/exceptions.py` | Application exception classes (taking `ErrorCode`) and runtime error-envelope builders |
| `app/core/error_codes.py` | `ErrorCode(str, Enum)` registry — the authoritative list of `DOMAIN.REASON` codes |
| `app/core/pagination.py` | `paginate_sequence` helper for slicing already-materialized in-memory lists |
