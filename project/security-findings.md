...
| F-040 | **Critical (broken feature)** | clients | `BinderRepository.close_in_office_by_client_record()` is called in `ClientUpdateService._update_client_record_status()` when freezing or closing a client, but the method does not exist in `BinderRepository`. Any attempt to freeze or close a client raises `AttributeError` and the entire transaction rolls back — client status is never updated, no VatWorkItems or AnnualReports are cancelled. | `app/clients/services/client_update_service.py:84` | open |
