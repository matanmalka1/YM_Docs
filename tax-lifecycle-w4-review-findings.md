...
**מצב נכון ל-30 ביולי 2026:** ממצאים 1, 2, 3, 4, 5, 6, 7, 9 ו-10 תוקנו ונכנסו לענף. ממצא 4 התברר בחקירה כשני באגים ולא אחד — פירוט בסעיף עצמו, לרבות שאלה פתוחה אחת שנשארה להכרעה. ממצאים 9 ו-10 נסגרו יחד עם ממצא 5, כי הם נוגעים באותם קבצים. **פתוחים: 8 ו-11.** ממצא 8 מראה שממצא 3 עדיין אינו סגור מקצה לקצה — ובפרט, ה-badge "נוכחי" בשלושת מודלי השרשרת עדיין נקבע לפי `superseded_at == null` בלבד, ולכן אחרי `canceled → create fresh` שתי שורות ייקראו כנוכחיות. תיקון ממצא 5 לא נגע בכלל הזה במכוון.
...
עדכון לאחר תיקון ממצאים 5, 9 ו-10:

- Frontend — 57 קובצי בדיקה ו-197 בדיקות עברו (`npm run check` מלא: TypeScript, ESLint, Prettier, arch-check, Vitest, Knip).
- Backend — `pytest tests/advance_payments tests/annual_reports tests/regression` — 326 עברו. הסוויטה המלאה לא הורצה: השינוי ב-Backend הוא שדה אחד בשני DTO של רשימה.
- סנכרון OpenAPI עבר; `ruff check` ו-`ruff format --check` על הקבצים שנגעו — נקי.
- בדיקת הניווט של ממצא 9 אומתה כרגרסיה אמיתית: היא נופלת על הקוד שלפני התיקון ועוברת עליו.
- עדיין אין כיסוי לממצאים 8 ו-11.
...
5. ~~להשלים את תהליך ה-Frontend בכל שלושת התחומים.~~ תוקן — יצירת תיקון קיימת בשלושת התחומים ומנווטת לרשומה החדשה, השרשרת ניתנת לפתיחה, והרשימות מסמנות תיקון.
...
9. ~~לתקן את הניווט אחרי ביטול תיקון מקדמה ואת invalidation שרשרת המע"מ.~~ תוקן יחד עם ממצא 5 — אותם קבצים.
