...
| SearchPage | `/search` | `features/search/pages/SearchPage.tsx` | list | 121 | low | 8.3 | pending | Client filter now hydrates display identity from URL-backed `client_record_id`, preserving client name and office number after reload; full page convergence still pending. Verify queryDraft — fix only if it duplicates URL state. |
