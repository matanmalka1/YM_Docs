...
| SearchPage | `/search` | `features/search/pages/SearchPage.tsx` | list | ~20 | low | 8.3 | done | Converged to a one-hook composition shell. `useSearchPage` owns URL/query orchestration, the intentional synchronized debounce draft, advanced-panel state, reset/focus behavior, result-state derivation, columns, and pagination. `SearchToolbar`, `SearchClientSummary`, and `SearchResultsSection` own the page slots. Client identity hydrates from URL; selected clients receive the shared status card and related operational previews. Primary client/binder results use one DB-paginated intersection query; direct operational links open the concrete entity. |
