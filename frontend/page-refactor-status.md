...
| SearchPage | `/search` | `features/search/pages/SearchPage.tsx` | list | 17 | low | 8.3 | done | One-hook composition shell (section-loading variant — NO PageStateGuard; filters stay visible while results load). Grouped contract (`status`/`headerProps`/`toolbar`/`results`); page is pure slot composition. `useSearchPage` owns URL state, both queries, the intentional synchronized debounce drafts, advanced-panel state, reset/focus, and all result-state derivation. **Not a table page** — no `buildColumns`, no `DataTable`; the page is the two-phase search contract (client resolution → one client's item feed, see `docs/domains/search.md`), rendered as `SearchClientMatches` rows + `SearchItemFeed`/`SearchItemRow`. Slot components: `SearchToolbar` (+`SearchFiltersBar`), `SearchResultsSection` (+`SearchClientMatches`, `SearchSelectedClient`, `SearchItemFeed`). MAT-63: selection derives from the resolved client only (`resolveSelectedClient`), never from the raw `client_record_id` string; `results.selected` is one slot carrying client heading **and** feed, so a feed can never render without the client it belongs to; filter/selection URL transitions are centralized in `utils/searchSelection.ts`. `page` intentionally serves both lists — they are mutually exclusive (chooser only when `total > 1`, feed only when `total === 1`). MAT-87: one place to type. The ID-number and binder-number inputs and the client picker were deleted — the term already matches both identifiers, and the picker was a second client search beside the one the page is; `client_record_id` stays as URL selection state written by choosing a match. The advanced panel now holds four selects and no free-text input, and stale `id_number`/`binder_number` params are cleaned out of old URLs. MAT-65: the advanced-filter panel opens when the URL flips to carrying advanced filters (deep link, back/forward), a manual collapse afterwards is respected, and the toggle exposes `aria-expanded`/`aria-controls` over an always-rendered panel. MAT-64: enum-backed URL params parse through their owning feature's guard (`utils/searchUrlValues.ts`) and unsupported values are cleaned out of the URL, never sent. Tests: `utils/searchSelection.test.ts`, `utils/searchUrlValues.test.ts`, `components/SearchResultsSection.test.tsx`, `components/SearchFiltersBar.test.tsx`, `components/SearchToolbar.test.tsx`, `api/search.api.test.ts` (SSR-markup + pure fns; no jsdom/RTL in the stack). |
...
| ClientDetailsPage | `/clients/:clientId` | `features/clients/pages/ClientDetailsPage.tsx` | detail | 193 | medium | 9 | pending | Phase 1 only — no routing change. MAT-95: added `binders` tab ("קלסרים", after documents) — route `/clients/:clientId/binders`, `ClientBindersTab` (binders feature) renders a paginated `DataTable` over the existing `GET /clients/{id}/binders`; columns = global `/binders` base columns minus the client-identity columns, no row actions (client endpoint's `BinderDetailResponse` carries no `available_actions`). |
