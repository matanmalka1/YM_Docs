...
`FilterPanel` supports `title`/`subtitle`/`icon` (header row; icon defaults to a funnel when `title`
is set) and `size` (defaults to `'sm'` = compact density across Select/DatePicker/search/client-picker
