...
- Area subfolders use the canonical UI-role vocabulary by default: `list/` (index view — table, columns, rows, row-actions, filters bar, stats/summary bar, bulk toolbar), `detail/` (single-item inline view — panels, info sections, tabs, cards), `drawer/` (the slide-over drawer container and the sub-parts that exist solely to compose it — drawer footer, drawer-only field/section blocks), `form/` (create + edit modals, form fields, wizard steps), `dialogs/` (small confirm/alert/delete dialogs), `shared/` (used across more than one area within the feature). Domain-named folders (e.g. annual-reports `tax/`, `season/`, `annex/`, `financials/`) are allowed only for genuinely distinct sub-domains; they are still `.tsx`-only.
- The drawer overlay lives in `drawer/`, not `detail/`: a `<Feature>...Drawer.tsx` that renders the shared `DetailDrawer` plus its drawer-exclusive sub-parts. Inline single-item content (tabs, info sections, cards shown on a detail page) stays in `detail/`. A feature whose only single-item surface is the drawer has a `drawer/` and no `detail/`.
- Hooks are `useCamelCase.ts`. A feature with enough hooks to warrant it groups data hooks into `hooks/queries/` (pure data-read, `useQuery`) and `hooks/mutations/` (pure data-write, `useMutation`); the page hook (`use<Feature>Page`), UI-state hooks, and composite hooks that do both reads and writes stay flat in `hooks/`. Smaller features keep `hooks/` flat.
