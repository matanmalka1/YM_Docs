...
- Tax-calendar completion/reopen and reminder synchronization on annual-report status transitions is described in legacy notes but is not implemented in the current `transition_status` path; treat it as planned or stale until code is changed.
...
- Income and expense update/delete endpoints accept `report_id` but the service updates/deletes by `line_id` after only checking that the report exists; it does not verify the line belongs to that report. This can mutate a line from a different report if a valid foreign `line_id` is supplied. See `backend/app/annual_reports/services/financial_service.py:171` and `backend/app/annual_reports/services/financial_service.py:268`. Suggested fix: repository queries for update/delete should include both `line_id` and `annual_report_id`, matching the annex service ownership check at `backend/app/annual_reports/services/annex_service.py:87`.
