...
| Route | Change | Regen? |
|---|---|---|
| `/audit/{type}/{id}` (`EntityAuditLogResponse`) | **`old_value`/`new_value` change from string → JSON object in Phase 1** (forced by the serializer switch); `actor_type`/`actor_display_name`/`metadata_json` added; registry authz | **yes — in Phase 1** for the string→object change (regen `openapi.json` + `generated.ts`, update `src/features/audit/` contracts + any old/new-value formatters, run frontend checks); additive auth/actor fields land with Phase 2. **Not deferred to Phase 10.** |
...
- **Serializer/reader switch** (§1b): drop `json.dumps` in `EntityAuditWriter._serialize_value` + `UserAuditLogRepository.create`; drop `json.loads` in `AuditLogService._to_dict`; `EntityAuditLogResponse`/`UserAuditLogResponse` type `old_value`/`new_value`/`metadata_json` as JSON objects.
...
- Acceptance: PostgreSQL up+down round-trip (1a+1b) clean and fail-safe; SQLite `create_all` works; existing audit writes pass `actor_type`/`actor_display_name`; backend suite green; generic-audit OpenAPI/types regenerated and **frontend audit area green**; legacy tables untouched.

**Phase 2 — Writer helpers / repository / registry / authz / validation.** Append-only audit repositories (§17 Option A); `EntityAuditWriter` (`record_action`/`record_external_action`, **actor validation matrix §5a**, **fail-closed write validation §16**); `EntityAuditLogRepository` client-context queries; `AuditEntityRegistry` + `resolve_scope` + route authorization using the **existing ADVISOR/SECRETARY + active/deleted client model** (§3a); read-time redaction hook (§16). Enrich existing client/business/charge/annual-report writes with required `metadata_json` (e.g. `client_record_id`); namespaced action constants. (Actor threading already done in Phase 1.) Acceptance: generic route authorizes (401/403-by-role/404/redaction + soft/hard-deleted scope tests); actor-validation + append-only + atomicity tests pass; existing writes carry required metadata.
