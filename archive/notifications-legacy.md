## Scope
This file owns only:
- Historical notification-domain material preserved from the old module README.

This file must not contain:
- Canonical current-state rules.
- Architecture/API rules.

Source of truth: historical

# Notification Legacy Notes

Archived from `backend/app/notification/README.md` during the canonical domain-doc migration on 2026-05-29.

## Historical overview

- The legacy README positioned notifications as the communication/audit layer for operational events across binders, annual reports, VAT, charges, signatures, and client-information workflows.
- It described a role split where advisors could preview/send manual notifications and secretaries could only view history/summary.
- It described the notification lifecycle as: trigger selection -> policy checks -> optional skipped record when no recipient exists -> `sent`/`failed` persistence after delivery.
- It documented that blocked policy outcomes should not create notification records.
- It documented that the automatic path was limited to binder handover.
- It described future expansion to additional channels beyond email.
