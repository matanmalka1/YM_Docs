...
3. **Export endpoint** (decided 2026-07-23, replaces the client-side CSV idea): `GET /advance-payments/overview/export` returns an xlsx Blob for the current filters, mirroring the existing clients export (`GET /clients/export`). No client-side file building — matches the server-side principle and the established pattern.
