...
3. **Export via the reports domain** (decided 2026-07-23, superseding both the client-side CSV idea and an overview-level export): `GET /reports/advance-payments/export` exports the existing collections report (year/month), mirroring the aging-report export. Exports live in the reports domain; the org list screen itself does not export.
4. **Column sorting via a server `order` param** on the overview endpoint + sortable headers in the table primitive. Rows are server-paginated (20/batch page), so client-side sorting would only reorder the visible page — misleading; no interim.
