...
12. **Office-wide annual schedule generation.** One action "צור לוחות שנה לכל הלקוחות": runs the existing per-client generator over all active clients with a configured frequency, skips existing periods, returns per-client created/skipped summary. Kills the December click-marathon (today the modal is per-client only). Spec'd 2026-07-26:
    ...
    - **Cursor-chunked, not one request.** `src/api/client.ts` sets a 15s axios timeout for every call, and the office is 80–400 clients × up to 12 periods — one synchronous request would be cut off mid-run. The backend owns the chunking: it picks the batch size and the ordering and returns a `next_cursor`; the frontend only repeats the call while `next_cursor !== null` and draws progress. The frontend must not choose which clients are in a batch — eligibility, ordering and batch size are domain knowledge and stay server-side. A real background job (job table + polling) was **rejected as disproportionate** at this scale: the app has no request-triggered job infrastructure (`app/core/background_jobs.py` holds only startup/daily loops), so it would be new infra for a once-a-year action.
    - **Preflight endpoint, used twice.** `GET /advance-payments/bulk-generate/preview` returns the eligible-client count plus the *ineligible* clients with a reason. Deliberately takes no `year`: eligibility is a property of the client (active, frequency configured), not of the year being generated. The modal shows it before the run ("210 לקוחות זכאים · 12 ללא תדירות מקדמות") and the same list is the warning block after it. One endpoint, two uses.
    - **Eligibility = `ClientStatus.ACTIVE` + a frequency on the legal entity.** Closed and frozen clients are skipped silently — `_assert_client_allows_create` already forbids creating an advance for them, so a bulk run must not attempt it either. Freezing is temporary; when the client is unfrozen the advisor runs the per-client generator. Clients with no `advance_payment_frequency` **are reported, not silently dropped**: they are the one class of skip that is a data-quality defect the advisor should go fix, and filtering them out of the query would hide it.
    - **Result screen shows the summary plus exceptions only.** Routine skips (period already exists, due date already passed — `generate_annual_schedule` handles both) collapse into a single count; they are high-volume and expected. A full per-client table was rejected: 200+ near-identical rows bury the handful that matter.
    - **Same button, same modal.** The existing header action opens `GenerateScheduleModal` with a `לקוח בודד | כל המשרד` segmented control; single-client mode is unchanged. Avoids a third primary button in the header and a near-duplicate modal.
    - **Explicit year select in the modal** (`getOperationalTaxYear()`, six years descending), matching the 1.11 rate-change modal, replacing today's hidden dependency on the page's year filter. Office-wide generation is run at the turn of a tax year, when the page filter still points at the outgoing one.
    - Permissions ADVISOR-only and one idempotency key per chunk, both matching the other bulk endpoints. The key must be deterministic per `(run, cursor)` so retrying a failed chunk is safe. Audit reuses `ACTION_ADVANCE_PAYMENT_CREATED` with a `source` marker — no new audit action or policy entry.
13. **Frequency-change cleanup in the generator.** When regenerating after a legal-entity frequency change, soft-delete unpaid future rows left over from the old frequency (audited); paid/partial rows are kept. Verified gap: `exists_for_period` matches on the exact `YYYY-MM` key, so stale rows from the old cadence otherwise linger alongside the new ones. Spec'd and built 2026-07-26:

    - **What actually breaks, both directions.** Bimonthly → monthly: the odd months collide with the old rows and are skipped while the even months are created, leaving February covered twice, six times over. Monthly → bimonthly: every new period collides, so nothing is created at all and the change silently does nothing.
    - **Deletion is opt-in, never a side effect of "generate".** `cleanup_stale_cadence` defaults to false on both the per-client and the office-wide request. The main reason is 1.12: the generator now runs across the whole office on one click, so an automatic cleanup could soft-delete thousands of rows nobody approved.
    - **An unconfirmed run generates nothing at all.** When removable stale rows exist and the flag is off, the generator returns empty rather than creating the periods the stale rows do not happen to occupy — creating "the rest" is exactly how a client ends up with both cadences covering the same month. The response reports the count, the UI asks, and the caller repeats with the flag. Clients with only *settled* stale rows are not blocked: no cleanup could help them, so generation proceeds and the count is reported.
    - **Past-due rows are out of reach however stale their cadence.** The sweep only covers periods whose effective due date is still ahead. An unpaid period that is already due is a debt, not a leftover, and the generator would not have recreated it anyway.
    - **Paid and part-paid rows are kept and counted separately.** `stale_cadence.settled` is its own number, not folded into `skipped`: a settled bimonthly January permanently occupies that `YYYY-MM` key, so that slice of the year stays on the old shape until someone resolves it by hand. It is the one outcome that needs a human, so it must not be buried among the routine skips.
    - Deletions reuse `advance_payment.deleted` with a system-written `reason` (that metadata key is required by the audit policy), so the change log shows what removed each row.
