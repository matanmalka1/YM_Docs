...
| 4 | Error code off `DOMAIN.REASON` format or off the registry | Codes are the `ErrorCode` enum in `app/core/error_codes.py`; `AppError` and subclasses take `ErrorCode`, not `str`. Flag any raw string passed as a code, any new code missing from the enum, and any code/message slot swap (code text in the message slot, or a non-Hebrew user message). | `docs/backend/error-codes.md`, `docs/architecture/api-contracts.md` |
