...
- Annual-report status transitions use the generic composed action `annual_report.status_changed`. Annual-report actions are pre-namespaced for detail/child edits: `annual_report.updated`, `annual_report.deadline_updated`, `annual_report.{income,expense}_line_{added,updated,deleted}`, `annual_report.annex_line_{added,updated,deleted}`.
...
- Binder actions are pre-namespaced (Phase 5). Lifecycle events anchor on `binder`: `created` (one row collapsing both initial statuses), `material_received`, `marked_full`, `reopened`, `marked_ready_for_handover`, `reverted_ready`, `handed_over` — the changed status is in `old_value`/`new_value` (`{"location_status"|"capacity_status": value}`), `metadata_json` carries `client_record_id`/`binder_id`/`binder_number`. Intake edits anchor on `binder_intake`: `updated` (one row per changed field; value wrapped as `{"value": ...}`, `metadata_json` carries `client_record_id`/`binder_id`/`intake_id`/`field_name`).
- Signature-request actions are pre-namespaced (Phase 6). Lifecycle/evidence events anchor on `signature_request`: `created`, `sent`, `viewed`, `signed`, `declined`, `canceled`, `expired`, and `annual_report_signed`. Signature rows use `actor_type=user` for advisor/internal user actions, `external_signer` for public signer actions, and `system` for automatic expiry/annual-report evidence. `metadata_json` carries the forensic context: `client_record_id`, `signer_name`, optional `signer_email`/`business_id`/`annual_report_id`/`document_id`; signer-facing `viewed`/`signed`/`declined` rows additionally carry `ip_address`/`user_agent` when available; `signed` carries `content_hash` when available or `content_hash_missing=true` when absent, plus `signed_document_key` when available; `canceled`/`expired` may carry `reason`.
- Phase-8 actions are pre-namespaced for the remaining live write surfaces: `legal_entity.created/updated`, `person.created/updated`, `person_legal_entity_link.created/deleted`, `authority_contact.created/updated/deleted`, `advance_payment.created/updated/deleted/turnover_refreshed`, `document.uploaded/updated/replaced/deleted`, `invoice.created`, `note.created/updated/deleted`, `task.created/updated/assigned/completed/canceled/deleted`, `correspondence.created/updated/deleted`, `notification.sent`, `reminder.created/canceled/fired/failed`, and `tax_calendar.generated`. `deadline_rule` remains excluded because there is no per-row UI edit route.
...
- **Same-transaction, fail-closed writes (Phase 2):** `EntityAuditWriter` validates every write (actor matrix + payload safety) and appends in the caller's transaction. An invalid payload or failed insert raises and rolls the domain mutation back; a rolled-back mutation leaves no orphan audit row. Annual-report status transitions rely on this behavior for `annual_report.status_changed`. The old `actor_id is None` no-op is removed. Source: `backend/app/audit/services/audit_entity_audit_writer_service.py`.
...
3. Generic audit inspection is registry-driven. As of Phase 8, the live write set includes `client`, `business`, `legal_entity`, `person`, `person_legal_entity_link`, `authority_contact`, `note`, `advance_payment`, `charge`, `invoice`, `vat_work_item`, `vat_invoice`, `annual_report`, `binder`, `binder_intake`, `document`, `signature_request`, `task`, `correspondence`, `notification`, `reminder`, and `tax_calendar`. `binder_handover` remains represented by binder lifecycle audit, and `deadline_rule` is excluded because there is no per-row UI edit route. `ALLOWED_READ_ENTITY_TYPES` is derived from the registry. Source: `backend/app/audit/audit_entity_registry.py`.
