...
- **`metadata_json.client_record_id`** is enriched on every existing client/business/charge/annual-report/VAT/binder/signature write (plus domain context such as `business_id`, `tax_year`, VAT period/work-item context, binder identity, and signature forensic fields where relevant) so client-context reads and the §8b expression index work.
...
3. Generic audit inspection is registry-driven. As of Phase 6, `client`, `business`, `charge`, `annual_report`, `vat_work_item`, `vat_invoice`, `binder`, `binder_intake`, and `signature_request` write generic entity audit rows. The registry additionally declares the full §6 entity set so reads/scope are defined for every audited type as later phases add their writes. `ALLOWED_READ_ENTITY_TYPES` is derived from the registry. Source: `backend/app/audit/audit_entity_registry.py`.
