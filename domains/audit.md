...
- Phase-8 actions are pre-namespaced for the remaining live write surfaces: `legal_entity.created/updated`, `person.created/updated`, `person_legal_entity_link.created/deleted`, `authority_contact.created/updated/deleted`, `advance_payment.created/updated/deleted`, `document.uploaded/updated/replaced/deleted`, `invoice.created`, `note.created/updated/deleted`, `task.created/updated/assigned/completed/canceled/deleted`, `correspondence.created/updated/deleted`, `notification.sent`, `reminder.created/canceled/fired/failed`, and `tax_calendar.generated`. `deadline_rule` remains excluded because there is no per-row UI edit route.
