...
- `AnnualReportStatusHistory` (`annual_report_status_history`): legacy append-only status-history table kept only until the Phase 9 cleanup migration. Phase 4 removed production writes/reads and the per-domain `GET /annual-reports/{report_id}/audit` route; status changes are written to generic `EntityAuditLog` as `annual_report.status_changed`.
