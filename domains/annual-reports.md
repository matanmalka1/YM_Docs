...
- Financial summary totals income and recognized expenses; `taxable_income = total_income - recognized_expenses`. See `backend/app/annual_reports/services/financial_service.py:315`.
- Tax calculation is read-only. It uses financial summary, detail deductions/credits, credit-point rows/default resident points, income-tax engine, national-insurance engine, VAT net balance, and paid advances. See `backend/app/annual_reports/services/financial_service.py:332`.
- Persisting a tax calculation rejects requests with both `tax_due` and `refund_due`. See `backend/app/annual_reports/services/financial_service.py:454`.
- Income and expense line create/update/delete are financial mutations. Closed/frozen clients must not create, update, or delete annual-report income/expense lines. See `backend/app/annual_reports/services/financial_line_helpers.py:39`.
- Income/expense source/category values are validated against enum values; audit entries are written for manual line mutations. See `backend/app/annual_reports/services/financial_service.py:115`.
- VAT auto-populate is a financial mutation and follows the same mutation rules as manual income/expense line changes. It requires a concrete actor id so audit cannot be silently skipped, and it is blocked for closed/frozen clients, including `force=True`; it writes audit records for created income lines, created expense lines, and deleted/replaced lines when `force=True`. Created-line audit payloads mark `source=vat_import`; force-replacement delete payloads mark `mutation_source=vat_import` and `mutation_reason=force_replace`. See `backend/app/annual_reports/services/vat_import_service.py:122`.
...
- Annual reports are owned by client record and tax year, not by business. The root model, list queries, and VAT/advance/tax calculations use `client_record_id` and `tax_year`. See `backend/app/annual_reports/models/annual_report_model.py:43` and `backend/app/annual_reports/services/financial_service.py:401`.
- Annual-report financial line mutations are locked for closed/frozen clients. This includes manual income/expense create, update, delete, and VAT auto-populate, including forced replacement.
