...
- `AnnualReport` (`annual_reports`): root aggregate. Required fields: `id`, `client_record_id` FK to `client_records.id`, `tax_year`, `client_type`, `form_type`, `status`, `deadline_type`, `tax_calendar_entry_id` FK to `tax_calendar_entries.id`, `created_at`, `updated_at`; optional fields: `created_by`, `assigned_to`, `filing_deadline`, `custom_deadline_note`, `ita_reference`, `assessment_amount`, `refund_due`, `tax_due`, `submission_method`, `extension_reason`, `notes`, `deleted_at`, `deleted_by`; closing facts (W3, written once when status → `submitted`): `closed_at`, `closed_by`, `closed_late` (NULL = no `filing_deadline` at close, D-32; otherwise the Israel-local date of `closed_at` vs the deadline as recalculated by the submission itself); non-null boolean flags default false: `has_rental_income`, `has_capital_gains`, `has_foreign_income`, `has_depreciation`, `has_exempt_rental`. See `backend/app/annual_reports/models/annual_report_model.py:42`.
...
- Readiness has five gates: an assignee set (the shared closing gate, D-15/W3), required schedules complete, total income greater than zero, either `tax_due` or `refund_due` persisted, and `AnnualReportDetail.client_approved_at` set. Completion percent is `passed / 5 * 100`. For an annual-report signature, the external signing timestamp is persisted as `client_approved_at` before submission readiness runs. The submission transition and sibling-signature cancellation are atomic; if another readiness gate fails, the report remains `pending_client` and startup/daily signature reconciliation retries after the blocker is corrected. See `backend/app/annual_reports/services/annual_report_readiness_service.py` and `backend/app/signature_requests/services/signature_request_service.py`.
