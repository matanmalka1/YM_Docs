...
**AC:**
- [ ] כל endpoint עם path param של ID כולל `404` ב-responses
- [ ] codegen מייצר טיפול ב-404

**התקדמות חלקית:** נוסף `ErrorEnvelope` ועוזר `not_found_response()` ב-`app/core/exceptions.py`; שני endpoints של audit (`GET /vat/work-items/{item_id}/audit`, `GET /audit/{entity_type}/{entity_id}`) כבר מתעדים `404`. נותר ה-sweep על שאר ה-endpoints.
